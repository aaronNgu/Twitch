...
function displayJSON(){
  ...
  for (var i = 0; i < JSONdata.length; i++){
      if (JSONdata[i].stream != null) {
    var name = JSONdata[i].stream.display_name,
        logo = JSONdata[i].stream.logo,
        status = "online",
        link = JSONdata[i]._links.self;
    $(".displays").prepend(makeDiv(name, logo, status, link));
  } else if(JSONdata[i].display_name != null) {

    var name = JSONdata[i].display_name,
        logo = "https://thumbs.dreamstime.com/b/cute-baby-pacifier-emoticon-child-soother-smiley-toddler-dummy-emoji-cute-baby-pacifier-emoticon-child-103935469.jpg",
        status = "offline",
        link = JSONdata[i]._links.self;
    $(".displays").append(makeDiv(name, logo, status, link));
  }
  }

}

function makeDiv(nm, lg , st, lk){
  var html = "<div class = 'display " + st + "'><img class = 'logo' src = '" + lg  +"'><a class = 'name' href ='" + lk + "' target = '_blank'>" + nm + "</a><span>"+ st + "</span></div>"
